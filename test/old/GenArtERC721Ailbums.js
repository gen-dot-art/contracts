...
const GenArtERC721Contract = artifacts.require("GenArtERC721Ailbums");
...
contract("GenArtERC721Ailbums", function (accounts) {
  before(async () => {
  ...
  it("Should get total supply", async () => {
    ...
    expect(supply.toString()).equals(String(COLLECTION_SIZE + 1));
  });
  ...
  it("Should do stress test", async () => {
    const size = 12;
    ...
    await genartERC721Contract.mint(user1, "5", {
      from: user1,
      value: MINT_PRICE.times(5),
    });
    const tx1 = await genartERC721Contract.mint(user2, "4", {
      from: user2,
      value: MINT_PRICE.times(5),
    });
    expectEvent(tx1, "Mint", {
    ...
    await genartERC721Contract.mint(user3, "2", {
      from: user3,
      value: MINT_PRICE.times(5),
    });

    const supply = await genartERC721Contract.totalSupply.call();
    expect(supply.toString()).equals(String(17));
  });
});
